...
public class Esquier : BasePiece
{
    public override void Setup(Color newTeamColor, Color32 newSpriteColor, PieceManager newPieceManager)
    {
        ...
        GetComponent<Image>().sprite = Resources.Load<Sprite>("T_Esquier");
    }
    ...
    protected override void CheckPathing()
    {
        ...
        // Top left
        if (MatchesState(currentX - mMovement.z, currentY + mMovement.z, CellState.Free))
        {
            // If the first forward cell is free, and first move, check for next
            if (mIsFirstMove)
            {
                MatchesState(currentX - (mMovement.z * 2), currentY + (mMovement.z* 2), CellState.Free);
            }
        }


        // Forward
        MatchesState(currentX, currentY + mMovement.y, CellState.Enemy);
